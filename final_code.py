...
N_FREQ = 1601                  # number of frequency points
N_ANTENNAS = 21                # number of antenna combinations / channels
EXPECTED_SIZE = N_FREQ * N_ANTENNAS
EPS = 1e-8                     # small number to avoid division by zero

BATCH_SIZE = 8

EPOCHS_SIM = 80                # training epochs on simulated data
EPOCHS_REAL = 70               # real fine-tuning with all layers open

LR_SIM = 1e-3                  # learning rate for simulated data
LR_REAL = 1e-3                 # low learning rate for full fine-tuning
...
def label_to_row_col(label):
    """
    Converts label 1-16 to grid coordinates 0-3.
...
def load_csv_last_column(path):
    """
    Loads simulated CSV data.
    Takes the last column, because that column contains the actual values.
    """
...
def load_sim_sample(mag_path, phase_path):
    """
    Loads one simulated sample from magnitude and phase CSV files.
    Converts magnitude + phase to a complex-valued matrix.
    """
...
def load_real_sample(path):
    """
    Loads one real sample from a .mat file.
    Expects the file to contain S21.
    """
...
class VNADataset(Dataset):
    """
    Dataset class for both simulated and real VNA data.
    Returns:
    x            -> input tensor, shape (2, 1601, 21)
    human_target -> 0 for empty, 1 for human
    pos_target   -> 0-15 for human position, -1 for empty
    label        -> 0-16 full class label
    """
    ...
    def load_raw(self, idx):
        """
        Loads raw sample before normalization.
        """
...
def stratified_split(dataset, train_ratio=0.70, val_ratio=0.15, seed=42): #due to little data
    """
    Splits data so every class gets represented in train/val/test.
    Better than random_split for classification.
    """
    ...
    for i, sample in enumerate(dataset.samples):
        label = sample[-1]
        if label_to_indices.get(label):
            label_to_indices[label].append(i) # key=label, value=index list
        else:
    ...
    for label, indices in label_to_indices.items():
        ...
        n = len(indices) # indices = list of indices for all elements from one class
...
def compute_normalizer(dataset, indices):
    ...
    This avoids data leakage from validation/test.
    """
...
def prepare_data(path, mode):
    """
    Creates dataset and train/val/test loaders.
    """
    ...
    mean, std = compute_normalizer(dataset, train_idx)
    dataset.set_normalizer(mean, std)
...
class VNAHybridCNN(nn.Module):
    """
    Hybrid model:
    - one head predicts empty/human
    - one head predicts position 1-16 if human exists
    """
...
def hybrid_loss(human_logit, position_logits, human_target, pos_target):
    ...
    if human_mask.sum() > 0:
        ...
        # normal position classification loss
        ...
        loss_distance = nn.MSELoss()(pred_xy, true_xy)

    else:
        loss_position = torch.tensor(0.0, device=human_logit.device)
        loss_distance = torch.tensor(0.0, device=human_logit.device)

    total_loss = loss_human + loss_position + 0.2 * loss_distance

    return total_loss, loss_human.item(), loss_position.item()
...
def train_model(model, loader, optimizer):
    ...
    for x, human_target, pos_target, _ in loader:
        ...
        loss, human_loss, position_loss = hybrid_loss(
            human_logit,
            position_logits,
            human_target,
            pos_target
...
def predict_labels(human_logit, position_logits, threshold=0.5):
    """
    Converts model outputs to final labels 0-16.
    """

    human_prob = torch.sigmoid(human_logit) #print this
...
def main():
    ...
    _, sim_train_loader, sim_val_loader, sim_test_loader = prepare_data(
        DATASET_SIM_PATH,
        mode="sim"
    )
    ...
    best_sim_val_acc = 0.0

    for epoch in range(EPOCHS_SIM):
        loss, human_loss, position_loss = train_model(
            model,
            sim_train_loader,
            optimizer
        )

        val_metrics = evaluate(model, sim_val_loader)

        if val_metrics[0] > best_sim_val_acc:
            best_sim_val_acc = val_metrics[0]
            torch.save(model.state_dict(), "model_sim_best.pth")

        print(
            f"[SIM] Epoch {epoch + 1:02d} | "
            f"Loss {loss:.4f} | "
    ...
    model.load_state_dict(torch.load("model_sim_best.pth", map_location=DEVICE))

    sim_test_metrics = evaluate(model, sim_test_loader)
    print_metrics("SIM Test", sim_test_metrics)

    print("\n=== FINE-TUNE ON REAL DATA ===")

    _, real_train_loader, real_val_loader, real_test_loader = prepare_data(
        DATASET_REAL_PATH,
        mode="real"
    )
    ...
    for epoch in range(EPOCHS_REAL):
        ...
        val_metrics = evaluate(model, real_val_loader)

        if val_metrics[0] > best_real_val_acc:
            best_real_val_acc = val_metrics[0]
            torch.save(model.state_dict(), "model_real_best.pth")

        print(
            f"[REAL] Epoch {epoch + 1:02d} | "
            f"Loss {loss:.4f} | "
            f"HumanLoss {human_loss:.4f} | "
            f"PosLoss {position_loss:.4f}"
        )
    ...
    model.load_state_dict(torch.load("model_real_best.pth", map_location=DEVICE))

    real_test_metrics = evaluate(model, real_test_loader)

    print_metrics("REAL Test", real_test_metrics)

    print("\nSaved best real model as: model_real_best.pth")
